...
def format_pending_list_for_telegram(pending_guests: List[models.Guest]) -> str:
    ...
    for i, guest in enumerate(pending_guests, 1):
        ...
        # --- SỬA LỖI (V3): Xử lý múi giờ chuẩn ---
        estimated_datetime_str = "N/A"
        if guest.estimated_datetime:
            try:
                # Đảm bảo datetime có timezone (aware)
                local_tz = pytz.timezone(settings.TZ)
                utc_tz = pytz.utc
                
                # Nếu là naive (chưa có tz), giả định là UTC (do frontend gửi UTC)
                if guest.estimated_datetime.tzinfo is None:
                     aware_utc_dt = utc_tz.localize(guest.estimated_datetime)
                     local_dt = aware_utc_dt.astimezone(local_tz)
                else:
                     # Nếu đã aware, chuyển đổi sang local time
                     local_dt = guest.estimated_datetime.astimezone(local_tz)
                
                estimated_datetime_str = local_dt.strftime("%d/%m %H:%M") # Format: 30/10 14:00

            except Exception as e:
                logger.warning(f"Lỗi xử lý múi giờ cho estimated_datetime (ID khách: {guest.id}): {e}")
